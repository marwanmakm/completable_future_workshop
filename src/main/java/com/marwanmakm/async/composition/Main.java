...
public class Main {
  ...
  private static void buildGeneral() throws ExecutionException, InterruptedException {
    ...
    CompletionStage<Soldier> result = cf.thenApply(soldier -> soldier.setRange(Soldier.Range.GENERAL));
  ...
  private static void buildGeneralWithFullArmor() throws ExecutionException, InterruptedException {
    ...
    cf
        ...
        .thenAccept(soldier::equipArmor) //<- Toma el objeto anterior y corre una funcion que no devuelve nada
        .thenRun(() -> System.out.println("El soldado está listo")) //<- No toma ningun objeto y solo ejecuta una funcion cualquiera
        .thenRun(() -> System.out.println(soldier));

    cf.toCompletableFuture().get();
  }

  private static void buildGeneralWithFullArmorV2() throws ExecutionException, InterruptedException {
    ...
    cArmor.toCompletableFuture().get();
  }
